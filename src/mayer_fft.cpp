...
void fft_filter::make_window(int n, float *window)
{
    ...
    float x = 2.0f * static_cast<float>(M_PI) * Ts;
    float tt = 0.0f;
    for(i=0; i<n; i++) {
...
//by adding the forward asymmetry
    for(i=100; i<128; i++) {
        ir[i]*=(0.5f-0.5f*cosf(static_cast<float>(M_PI)*(float)(i-100)/27.0f));
        ir[i]*=(0.5f-0.5f*cosf(static_cast<float>(M_PI)*(float)(i-100)/27.0f)); //square it
    }
    for(i=128; i<256; i++) {
        ir[i]*=(0.5f-0.5f*cosf(2.0f*static_cast<float>(M_PI)*(float)i/256.0f));
    }
