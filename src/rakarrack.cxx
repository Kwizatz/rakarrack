...
Fl_Menu_Item RKRGUI::menu_MenuP[] = {
 ...
 {"New", 'n',  (Fl_Callback*)RKRGUI::cb_Menu_New, 0, 128, (uchar)FL_NORMAL_LABEL, 0, 14, 0},
 {"Load Preset", 'l',  (Fl_Callback*)RKRGUI::cb_Load_Preset, 0, 0, (uchar)FL_NORMAL_LABEL, 0, 14, 0},
 {"Save Preset", 's',  (Fl_Callback*)RKRGUI::cb_Save_Preset, 0, 128, (uchar)FL_NORMAL_LABEL, 0, 14, 0},
 ...
 {"Exit", 'x',  (Fl_Callback*)RKRGUI::cb_salir, 0, 0, (uchar)FL_NORMAL_LABEL, 0, 14, 7},
 {0,0,0,0,0,0,0,0,0},
 {"&Bank", 'b',  (Fl_Callback*)RKRGUI::cb_Bank_Menu, 0, 0, (uchar)FL_NORMAL_LABEL, 0, 14, 0},
 {"&Settings", 0,  0, 0, 64, (uchar)FL_NORMAL_LABEL, 0, 14, 0},
 {"Preferences", 'i',  (Fl_Callback*)RKRGUI::cb_Ajustes, 0, 128, (uchar)FL_NORMAL_LABEL, 0, 14, 7},
 {"MIDI Learn", 'e',  (Fl_Callback*)RKRGUI::cb_ML_Menu, 0, 128, (uchar)FL_NORMAL_LABEL, 0, 14, 7},
...
Fl_Menu_Item RKRGUI::menu_shelf_preset[] = {
 {"Trebble", 0,  0, 0, 0, (uchar)FL_NORMAL_LABEL, 0, 10, 0},
...
void RKRGUI::cb_L_B1_i(Fl_Button*, void*) {
  is_modified();
  int ok=rkr->loadbank_from_memory(res_Default_rkrb, res_Default_rkrb_len);
  if(ok) 
  {
  rkr->presets.a_bank=0;
  BankWin_Label("Default");
  Put_Loaded_Bank();
...
void RKRGUI::cb_L_B2_i(Fl_Button*, void*) {
  is_modified();
  int ok=rkr->loadbank_from_memory(res_Extra_rkrb, res_Extra_rkrb_len);
  if(ok) 
  {
...
void RKRGUI::cb_L_B3_i(Fl_Button*, void*) {
  is_modified();
  int ok=rkr->loadbank_from_memory(res_Extra1_rkrb, res_Extra1_rkrb_len);
  if(ok) 
  {
  rkr->presets.a_bank=2;
  BankWin_Label("Extra1");
  Put_Loaded_Bank();
  unlight_preset(rkr->presets.Selected_Preset);
  }
}
...
void RKRGUI::cb_BMidiIn_i(Fl_Browser* o, void*) {
  ...
  strcpy(rkr->MID.data(),o->text(valor));
  else return;
  o->select(valor,1);
#ifdef ENABLE_MIDI
    rkr->Conecta();
...
void RKRGUI::cb_ClearP_i(Fl_Button*, void*) {
  memset(rkr->XUserMIDI.data(), 0, sizeof(rkr->XUserMIDI));
  DisAssigns();
}
...
"R average.");
    ...
    { SYNTHFILTER = new Fl_Group(319, 211, 158, 184);
      ...
      { synthfilter_activar = new Fl_Light_Button(324, 215, 34, 18, "On");
        synthfilter_activar->shortcut('7');
...
"e a stompbox Opto Trem");
  ...
  { MIDILearn = new Fl_Double_Window(640, 480);
    ...
    { TPresets = new Fl_Browser(430, 61, 201, 348);
      ...
      TPresets->when(FL_WHEN_CHANGED | FL_WHEN_NOT_CHANGED);
    } // Fl_Browser* TPresets
...
RKRGUI::RKRGUI(int argc, char**argv,RKR *rkr_) {
  ...
  Sco->init(rkr->anall.data(), rkr->analr.data(), PERIOD);
...
void RKRGUI::reordena() {
  ...
  switch (rkr->efx_order[i])
   ...
   {
      case 0:
         L1->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L1->hide(); else L1->show();
         break;
      case 1:
         L2->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L2->hide(); else L2->show();
         break;
      case 2:
         L3->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L3->hide(); else L3->show();
         break;
      case 3:
         L4->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L4->hide(); else L4->show();
         break;
      case 4:
         L5->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L5->hide(); else L5->show();
         break;
      case 5:
         L6->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L6->hide(); else L6->show();
         break;
      case 6:
         L7->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L7->hide(); else L7->show();
         break;
      case 7:
         L8->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L8->hide(); else L8->show();
         break;
      case 8:
         L9->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
         if((rkr->deachide) && (!rkr->active[i])) L9->hide(); else L9->show();
         break;
      case 9:
         L10->copy_label(rkr->efx_names[Busca_Eff(rkr->efx_order[i])].Nom.data());
...
void RKRGUI::MiraConfig() {
  ...
  {
  if (strcmp(BMidiIn->text(i),rkr->MID.data())==0 ) BMidiIn->select(i,1);
  i++; 
  }
...
void RKRGUI::PutBackground() {
  delete back;
  auto* epng = find_embedded_png(rkr->config.BackgroundImage.data());
  Fl_PNG_Image* bgimg;
  if (epng)
      bgimg = new Fl_PNG_Image("embedded", epng->data, *epng->len);
  else
      bgimg = new Fl_PNG_Image(rkr->config.BackgroundImage.data());
  back = new Fl_Tiled_Image(bgimg,1600,1200);
...
void RKRGUI::FillML(int type) {
  ...
  switch(rkr->ML_filter) 
  {
   ...
   case 0:
   for(i=0; i<rkr->NumParams; i++) Epar->add(rkr->efx_params[i].Nom.data());  
   break;
   ...
   for(i=0;i<rkr->NumParams;i++)
        {
          if(rkr->efx_params[i].Effect==50)
           { 
           Epar->add(rkr->efx_params[i].Nom.data()); 
...
void RKRGUI::Fill_Avail(int filter) {
  ...
  for (i=0; i<rkr->NumEffects;i++)
  {
   ...
   { 
    Avail_Bro->add(rkr->efx_names[i].Nom.data());
...
int RKRGUI::AddBankName(const char *name) {
  struct list_element *elem;
          ...
          if (mBankNameList == NULL) {
                  mBankNameList = elem;
                  elem->forward = elem->backward = nullptr;
          } else {
            mBankNameListTail->forward = elem;
            elem->backward = mBankNameListTail;
            elem->forward = nullptr;
          }
...
void RKRGUI::ClearBankNames() {
  struct list_element *elem;
          while (mBankNameList != NULL) {
                  ...
                  free(elem->name);
                  if(elem->forward) { elem->forward->backward = elem->backward; }
                  if(elem->backward) { elem->backward->forward = elem->forward; }
                  free(elem);
          }
}
